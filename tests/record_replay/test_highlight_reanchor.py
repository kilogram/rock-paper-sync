"""Test for highlight re-anchoring after X-shift, Y-shift, and multi-line reflow.
...
Recording:
    uv run pytest tests/record_replay/test_highlight_reanchor.py --online -s

Replaying:
    uv run pytest tests/record_replay/test_highlight_reanchor.py
"""
...
def test_highlight_reanchor(device, workspace, fixtures_dir):
    ...
    This validates that highlight re-anchoring works for horizontal shifts, vertical
    shifts, and the critical case where a highlight spans multiple lines after reflow.
    """
    fixture_doc = fixtures_dir / "test_highlight_reanchor.md"
